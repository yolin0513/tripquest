...
route('/trip/:id/nearby', async ({ params }) => (await import('./views/nearby.js')).default(params.id));
...
setNotFound(() => {
  ...
  // v1.73.6：**不要無聲無息**。使用者看到的是「按了就被踢走」，
  // 而這條保險絲的觸發條件其實很明確（載入中的程式是舊版），
  // 那正好是一句他做得到的指示。沒有這句話，他只會覺得 App 壞了。
  toast('這個畫面還沒更新好，先回到上一層。把 App 關掉重開就會好', 5000);
