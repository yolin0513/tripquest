...
async function _enrich(spot) {
  ...
  try {
    ...
    if (!spot.heroHash && !got && (!page || page.disambig)) {
      ...
      // 這裡試過「拿掉分店後綴（奕順軒 礁溪店 → 奕順軒）再搜一次」，撤掉了。
      // 命中只多一個，而那一個是「火山爆發雞礁溪總店」抓到冰島火山灰的照片 ——
      // related() 的規則是「店名包含條目標題就算相關」，而「火山爆發雞」確實
      // 包含「火山爆發」。收緊那條規則會連「清水寺 本堂 → 清水寺」一起殺掉。
      // 沒有圖會顯示主題色塊，那是誠實的；給錯的圖比沒有更糟。
