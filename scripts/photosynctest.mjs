...
//
// 涵蓋的程式：js/photoimg.js —— 縮圖優先、`.ph-state` 的狀態與「重新下載」；
// 另有 js/photos.js 與 js/outbox.js 。寫出來是給挑選器認的 —— 以前這支測試在跑
// photoimg.js 的行為，但沒有任何文字提到它，挑選器因此把它算成「沒人認領的程式檔」
// 而每次改到它都保守跑全套（那不是守護，是沒人看）。
//
