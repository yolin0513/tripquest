...
- **拍照或從相簿選**：任務頁與景點頁的每一列任務都有並排的「📷 拍照」與「🖼️ 從相簿選」兩顆大按鈕——**兩顆而不是一顆再跳選單**，長輩少按一層，而且按鈕上的字就直接說明會發生什麼。任務列上沒有別的東西可以按錯（實測長輩會下意識按「編輯」，以為那是加照片的入口），任務的編輯／刪除／新增集中在「調整每天的行程」那一頁。已完成的任務照樣可以補拍或再加。匯入時一律壓縮、清 EXIF、預設丟掉精確 GPS。
