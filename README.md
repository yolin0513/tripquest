...
- **成就徽章 + 照片標記**：標記**在上傳之後才做**——點照片牆上任一張照片，勾「照片裡有誰」（可複選）與「這張是誰拍的」，點名字就存好、不必按儲存，還能一路「下一張」連續標完。上傳時完全不問，因為一次選多張時那個問題會被**統一套用到每一張**（第一張全家福、第二張只有弟弟，結果弟弟那張也被標成全家）。未標記的照片有小小的「未標記」角標，照片牆上有「還有 N 張沒標記」的入口。統計與徽章一律以**最新的標記**為準，改完立刻重算。14 種徽章依實際行為自動解鎖（初次登場、一日三響、美食獵人、神隊友、早鳥、夜貓子、眾星拱月、全家福、全員通關、大導演、旅行常客…），達成時跳慶祝。照片牆每個人顯示完成數／拍攝數／幫拍／入鏡。
  - 標記存成獨立的 `phototag` 記錄、id 固定為 `tag:<投稿id>`，**不是**改投稿本身：投稿是 append-only，合併時對已存在的投稿直接跳過，標記若寫回投稿裡會在自己手機上看起來有改、卻永遠傳不到旅伴手機。固定 id 讓兩台標同一張時走一般的「後寫入者勝」合併。
...
- **拍照或從相簿選**：**行程頁的每一張任務卡上就直接有**並排的「📷 拍照」與「🖼️ 從相簿選」兩顆大按鈕 —— 打開行程到開始加照片**只要 1 下**。任務清單上沒有「編輯」（它會把長輩帶到景點頁，而拍照按鈕在那裡，於是「編輯」看起來就成了加照片的入口）。任務頁與景點頁的每一列任務也都有同樣兩顆大按鈕——**兩顆而不是一顆再跳選單**，長輩少按一層，而且按鈕上的字就直接說明會發生什麼。任務列上沒有別的東西可以按錯（實測長輩會下意識按「編輯」，以為那是加照片的入口），任務的編輯／刪除／新增集中在「調整每天的行程」那一頁。已完成的任務照樣可以補拍或再加。匯入時一律壓縮、清 EXIF、預設丟掉精確 GPS。
- **清楚的分頁**：底部功能列依所在位置切換——首頁是「我的旅程 / 設定」，進到旅程裡是「📋 任務 / 📸 照片 / 💰 分帳 / 🎁 回顧」。回憶影片、成就徽章、最終回顧、行程海報都收在「回顧」分頁（影片與回顧要全部完成或旅程結束才開放）。緊急求助只留右下角紅色 🆘。
- **兩層折疊，畫面乾淨**：任務清單分「天」和「景點」兩層折疊。天的標題列顯示日期、景點數、當天完成進度、天氣摘要。預設展開規則：**天** —— 進行中只展開今天、尚未出發只展開第 1 天、已結束或全部完成則全收；**景點** —— 還沒完成的展開、已完成的收起（做完的自動讓位，進來就看得到還要做什麼）。兩層的手動展開/收合都會記住並永遠優先。「全部展開 / 全部收合」**兩層一起**作用（只收「天」的話，展開後裡面景點還是關的，等於沒有全部展開），之後個別點過的仍以那次為準。收合整天時裡面景點的展開狀態會保留。
- **自動捲到今天**：旅行途中打開旅程，畫面會自動移到「今天」那一段。刻意保守，長輩最怕畫面自己亂跳——只在旅程進行中、今天那列本來就在畫面外時才動；只捲到「第 N 天」的**標題列**（讓人知道自己在哪一天），不捲進任務深處；平滑但短（320ms），一碰畫面就讓給使用者；系統的「減少動態效果」偏好開著時直接跳過去不做動畫。**回到看過的畫面一律不捲**——按返回、或底部分頁切回任務頁，都會還原原本捲到哪裡（`router.js` 記住每個畫面的捲動位置）。設定裡可整個關掉，預設開啟。
- **用地圖帶路**：每個景點都有大按鈕開 Google Maps——「用地圖看在哪裡」「用地圖帶我去…」。行程頁還有「用地圖帶我去下一站」，一鍵導航到當天下一個還沒完成的景點。連結**優先用地名不用座標**（會帶城市前綴、日本景點用日文名提高命中率），只有自訂無名或太通用的地點才退回座標；手機有裝 App 會直接喚起。
...
- **景點示意圖**：任務卡會顯示「要拍的東西長怎樣」的參考照片，抓回來存進 IndexedDB，**離線也看得到、之後開畫面不再重抓**。全部用免金鑰的 Wikimedia 系列，依序試：維基頁面圖 → 換當地語言（日本景點用日文命中率高）→ 維基站內搜尋（「清水寺 本堂」會帶到「清水寺」）→ Commons 座標搜尋（該座標附近的公開照片）。實測涵蓋率：策展地點 **100%**、自由輸入的景點名 **90%**。
  - **美食任務**用「該道菜」的通用照片（店家照片查不到），圖上明確標「示意圖」，不會被當成該店實照。涵蓋率 **84%**。
  - 都查不到就用該景點主題的色塊插畫佔位（`themePlaceholder()`，純本機 SVG），不留白。刻意**不**拿景點名去 Commons 做關鍵字搜尋 —— 實測 155 個真實地名一次都沒靠它命中，但查無此地的名字反而會撈到毫不相干卻剛好有圖的檔案，給錯的示意圖比沒有更糟。
  - Wikimedia 多為 CC 授權，圖片下緣以小字標作者與授權條款（例：`Martin Falbisoner · CC BY-SA 4.0`），資料來自 Commons `imageinfo` 的 `extmetadata`。
  - **只要真實照片**：排除 SVG／PDF，排除檔名或分類含 icon、logo、clip art、symbol、pictogram、diagram、map、flag、coat of arms、illustration… 的檔案，排除過小（< 320×200）與細長條的圖。美食更嚴格，只收 JPEG（Commons 上的 PNG 多半是書封或標題圖）。**寧可退到主題色塊，也不要放看起來像剪貼畫的圖** —— 那比佔位更廉價。
  - **排序以搜尋相關度為主、畫質為輔**：畫質分數最多把一張圖往前推一兩名。純比畫質會出事——搜「包心粉圓」時，第三名是同名文學獎的頒獎合照，解析度比那道甜點高得多。
  - **同一景點的任務不會重複同一張**：景點會從同一次查詢的候選多存幾張備用圖（`heroPool`，不多打 API），各任務依序輪流；美食則記住同景點已用過的圖，換下一張（「阿灶伯羊肉湯」和「當歸羊肉湯」都會抽到「羊肉湯」）。
  - **舊行程會自動補**：判斷依據是每個景點的 `_enrichV`，舊資料沒有這個欄位就算 0，所以下次打開行程頁自然會補，使用者不用做任何設定（「景點示意圖」開關預設就是開的）。抓好一張就換上那一張，不整頁重畫。
  - **同步過來的旅程也會自己補圖**：圖存在各自手機的 IndexedDB、不跟著同步，但記錄（`heroHash`）會同步。所以旅伴的手機拿得到 hash 卻沒有圖 —— 記錄裡另存 `heroUrl`，開啟時自動重抓一次。出處標示也改成**等圖真的載出來才顯示**，否則會變成「大大的 emoji ＋ 別人的姓名授權」。
  - **沒網路時安靜略過、不留標記**：分得出「查了但沒有」與「根本連不上」（看有沒有真的收到 Wikimedia 的回應）。連不上就什麼都不寫，之後有網路再補；斷網期間畫面用主題色塊撐住，不會空白。
  - 消歧義頁會自動追到真正的條目（中文維基的「金閣寺」是消歧義頁，真正的條目是「鹿苑寺」），而且那句「金閣寺可以指：」不會被拿去當景點介紹。
  - 涵蓋率可用 `npm run imgcoverage` 重新量測。
...
| 模式 | 做什麼 |
|---|---|
| **Cloudflare**（預設，已上線） | Workers + R2 + D1，網址內建在 `js/sync.js` 的 `BUILT_IN`。使用者什麼都不用設定，開 App 就自動同步。免費、永遠開著、對外流量不收費 |
| **自架伺服器** | `node server/index.mjs`（零相依）＋ Cloudflare Tunnel 打外網。不需綁卡，但電腦要保持開機 |
| **單機** | 在設定頁關掉同步。分享「邀請連結」給旅伴（含行程 + 任務，數 KB 的 `d=` 代碼）；用「匯出 / 匯入備份」合併照片 |
...
- **邀請連結只帶群組識別碼**（約 300–400 字元），資料由加入端自己向伺服器拉。早期版本把整包行程塞進網址，行程一大就長到 LINE 傳不動、貼上還會漏字。
- **D1 的地雷**：每條查詢最多約 100 個綁定參數、`batch()` 也有陳述式數上限，超過時 Worker 會被平台直接中止（不是可攔截的 JS 例外，回應還沒有 CORS 標頭，瀏覽器只會顯示看不懂的「CORS blocked」）。`workers/worker.mjs` 裡所有批次操作都抓在 **≤99**。
- 資料模型：UUID + `updatedAt`/`deviceId`，中繼資料「後寫入者勝 + 墓碑」，投稿 / 讚 / 留言 / 撤回**只新增**（合併即集合聯集，永不衝突），同步游標用伺服器指派的序號。

**同步後端已部署上線**（<https://tripquest.yolin0513.workers.dev>），使用者不需要做任何設定。重新部署 / 換帳號的步驟見 [`SETUP_TODO.md`](./SETUP_TODO.md)。
...
js/
  ...
  maps.js                  以地名（非座標）組 Google Maps 導航連結，座標只當退路
  ...
  badges.js                成就徽章推導（14 種）+ 依標記歸屬 + 新達成偵測
  phototag.js              照片標記畫面（照片裡有誰 / 誰拍的，可連續標下一張）
  addphoto.js              「拍照 / 從相簿選」共用流程（任務頁與景點頁）
  ...
  views/plan.js            總行程編輯：拖曳 / 大按鈕調整景點的天與順序，任務的編輯 / 刪除 / 新增
