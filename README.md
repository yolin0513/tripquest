...
- **兩層折疊，畫面乾淨**：任務清單分「天」和「景點」兩層折疊。天的標題列顯示日期、景點數、當天完成進度、天氣摘要。預設展開規則：**天** —— 進行中只展開今天、尚未出發只展開第 1 天、已結束或全部完成則全收；**景點** —— 還沒完成的展開、已完成的收起（做完的自動讓位，進來就看得到還要做什麼）。兩層的手動展開/收合都會記住並永遠優先。有「全部展開 / 全部收合」小工具。收合整天時裡面景點的展開狀態會保留。
